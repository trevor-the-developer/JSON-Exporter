...
public class AdvancedJsonPathParser
{
    ...
    private static List<JsonElement> ApplySegment(List<JsonElement> elements, string segment)
    {
    ...
    private static List<JsonElement> HandleArrayOperation(JsonElement element, string segment)
    {
        ...
        if (sliceMatch.Success)
        {
            ...
            var end = string.IsNullOrEmpty(endStr) ? array.Length : int.Parse(endStr);
    ...
    private static JsonElement? ExecuteFunction(JsonElement element, string functionName)
    {
        ...
        {
            "length" => CreateNumberElement(GetLength(element)),
            "keys" => CreateArrayElement(GetKeys(element)),
            "values" => CreateArrayElement(GetValues(element)),
            _ => null
        };
    }
